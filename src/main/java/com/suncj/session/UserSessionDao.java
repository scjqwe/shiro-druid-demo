...
import com.suncj.constant.SystemConstant;
import com.suncj.entity.UserSession;
import com.suncj.util.RedisUtil;
import com.suncj.util.SerializableUtil;

@Component
public class UserSessionDao extends CachingSessionDAO {
	private static Logger _log = LoggerFactory.getLogger(UserSessionDao.class);
	// 会话key
	private final static String ZHENG_UPMS_SHIRO_SESSION_ID = "demo-shiro-session-id";
	// 全局会话key
	private final static String ZHENG_UPMS_SERVER_SESSION_ID = "demo-server-session-id";
	// 全局会话列表key
	private final static String ZHENG_UPMS_SERVER_SESSION_IDS = "demo-server-session-ids";
	// code key
	private final static String ZHENG_UPMS_SERVER_CODE = "demo-server-code";
	// 局部会话key
	private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "demo-client-session-id";
	// 单点同一个code所有局部会话key
	private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "demo-client-session-ids";
	...
	protected void doUpdate(Session session) {
		...
		UserSession cacheUserSession = (UserSession) doReadSession(session.getId());
		if (null != cacheUserSession) {
			userSession.setStatus(cacheUserSession.getStatus());
			userSession.setAttribute("FORCE_LOGOUT", cacheUserSession.getAttribute("FORCE_LOGOUT"));
		}
	...
	protected void doDelete(Session session) {
		...
		String upmsType = ObjectUtils.toString(session.getAttribute(SystemConstant.USER_TYPE));
	...
	public int forceout(String ids) {
		...
		for (String sessionId : sessionIds) {
			...
			String session = RedisUtil.get(ZHENG_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
			UserSession userSession = (UserSession) SerializableUtil.deserialize(session);
			userSession.setStatus(UserSession.OnlineStatus.force_logout);
			userSession.setAttribute("FORCE_LOGOUT", "FORCE_LOGOUT");
			RedisUtil.set(ZHENG_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(userSession), (int) userSession.getTimeout() / 1000);
		}
	 ...
	 * @param onlineStatus
	 */
	public void updateStatus(Serializable sessionId, UserSession.OnlineStatus onlineStatus) {
		UserSession session = (UserSession) doReadSession(sessionId);
		if (null == session) {
			return;
		}
